...
if config["alignment_backend"] == "legacy":
    rule fastp:
        input:
        ...
        output:
            ...
            report_html = os.path.join(DATA_DIR,"Report","fastp","{sample}_run{run}_fastp.html"),
            report_json = os.path.join(DATA_DIR,"Report","fastp","{sample}_run{run}_fastp.json"),
        params:
